...
{
    public partial class Chamber5_ViewModel : ObservableObject
    {
        #region FIELDS
        ...
        public Chamber5_ViewModel(ILogManager logService, IMessageBox messageBox, IChamberManager chamberManager, IEquipmentConfigManager equipmentConfigManager, IPLCManager plcManager)
        {
            this._logManager = logService;
            // 구독: 로그가 갱신될 때마다 OnLogUpdated 호출
            this._logManager.Subscribe($"Dry_Chamber5", OnLogUpdated);
            ...
            this.StatusText = this._chamberManager.Chamber_State["Chamber5"];
        ...
        private void SetupLogFileWatcher()
        {
            ...
            var logFileName = $"Dry_Chamber5_{DateTime.Now:yyyyMMdd}_{DateTime.Now:HHmmss}.log";
        ...
        {
            var logPath = Path.Combine(@"C:\Logs", $"Dry_Chamber5_{DateTime.Now:yyyyMMdd}_{DateTime.Now:HHmmss}.log");
        ...
        private void OnDataEnqueued(object sender, ChamberStatus chamber)
        {
            ...
            if (chamber.ChamberName == "Chamber5")
            {
        ...
        {
            if (e.CurrentLocation == "Dry Chamber_Chamber5")
            {
                if (Application.Current.Dispatcher.CheckAccess())
                {
                    lock (_itemLock1)
